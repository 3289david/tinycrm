  ...
  return (
    <div className="max-w-xl mx-auto px-8 py-10">
      ...
      <form action={async (fd: FormData) => { 'use server'; await createClient_action(fd) }} className="space-y-4">
        ...
        <Field name="tags" label="Tags" placeholder="VIP, Design, Personal (comma separated)" />
        <div>
          <label className="block text-xs text-ink-mute mb-1.5 font-normal">Notes</label>
          <textarea
            name="notes"
            rows={4}
            className="w-full px-3 py-2.5 rounded-sm border border-hairline-input text-ink text-sm bg-canvas outline-none focus:border-primary transition-colors resize-none"
